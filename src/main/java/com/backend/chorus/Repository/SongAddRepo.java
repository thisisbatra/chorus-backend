...
public interface SongAddRepo extends CrudRepository<Song, Long> {
    ...
    // native Query
    @Query(value = "SELECT * FROM chorus.song_collection ORDER BY like_count DESC LIMIT 10", nativeQuery = true)
    public List<Song> mostLikedSongs();

    @Query(value = "SELECT * FROM chorus.song_collection ORDER BY click_count DESC LIMIT 15;", nativeQuery = true)
